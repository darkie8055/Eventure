  ...
  return (
    <div className="min-h-screen h-screen flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className={`hidden lg:flex lg:w-1/2 bg-gradient-to-br ${content.gradientFrom} ${content.gradientVia} ${content.gradientTo} relative overflow-hidden min-h-screen`}>
        ...
        <div className="absolute top-16 left-12 w-40 h-40 bg-white/10 backdrop-blur-md rounded-full border border-white/20 shadow-2xl"></div>
        <div className="absolute top-1/3 right-16 w-32 h-32 bg-white/15 backdrop-blur-lg rounded-2xl border border-white/20 shadow-xl rotate-12"></div>
        <div className="absolute bottom-24 left-20 w-24 h-24 bg-white/10 backdrop-blur-md rounded-full border border-white/20 shadow-lg"></div>
        <div className="absolute bottom-1/3 right-8 w-16 h-16 bg-white/20 backdrop-blur-sm rounded-lg border border-white/30 shadow-md rotate-45"></div>
        
        {/* Dynamic floating elements based on page type */}
        {isLogin && (
          <>
            <div className="absolute top-20 right-24 w-28 h-28 bg-gradient-to-br from-pink-400/30 to-purple-400/20 rounded-full blur-xl animate-pulse"></div>
            <div className="absolute bottom-32 left-16 w-36 h-36 bg-gradient-to-br from-indigo-400/25 to-purple-500/15 rounded-full blur-2xl"></div>
          </>
        )}
        
        {isSignup && (
          <>
            <div className="absolute top-24 right-20 w-32 h-32 bg-gradient-to-br from-emerald-400/30 to-purple-400/20 rounded-full blur-xl animate-pulse"></div>
            <div className="absolute bottom-28 left-12 w-40 h-40 bg-gradient-to-br from-purple-400/25 to-indigo-500/15 rounded-full blur-2xl"></div>
            <div className="absolute top-1/2 left-1/3 w-20 h-20 bg-gradient-to-br from-blue-400/20 to-purple-400/15 rounded-full blur-lg"></div>
          </>
        )}
        ...
        {/* Subtle campus/community icons - minimal abstract shapes */}
        <div className="absolute top-1/4 left-1/4 w-6 h-6 bg-white/20 rounded border border-white/30 rotate-45"></div>
        <div className="absolute bottom-1/4 right-1/3 w-4 h-4 bg-white/15 rounded-full"></div>
        <div className="absolute top-2/3 right-1/4 w-5 h-5 bg-white/20 rounded border border-white/25 rotate-12"></div>
          ...
          <div className="max-w-lg space-y-8">
            ...
            <div className="flex items-center space-x-4 mb-12">
              ...
              <div>
                ...
                <p className="text-sm text-white/80">Campus Community Platform</p>
              </div>
            </div>
            ...
            <div className="space-y-5 mt-12">
              {content.features.map((feature, index) => (
                <div key={index} className="flex items-start space-x-4 group">
                  ...
                  <span className="text-lg text-white/95 group-hover:text-white transition-colors duration-200">
                    {feature}
                  </span>
                </div>
              ))}
